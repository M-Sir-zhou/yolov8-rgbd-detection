...
class BaseTransform:
    ...
    def apply_image(self, labels):
        ...
        if hasattr(self, "rgbd_mode") and self.rgbd_mode:
            ...
            # 重新合并通道
            labels["img"] = np.concatenate([rgb_channels, depth_channels], axis=-1)
        else:
            # 原有的单图像处理逻辑
            pass
...
class RandomHSV:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        img = labels["img"]
        # 如果是RGB-D图像，只对前3个通道(RGB)应用HSV变换
        if img.shape[-1] > 3:
            rgb_img = img[..., :3]
            depth_img = img[..., 3:]
            ...
            if self.hgain or self.sgain or self.vgain:
                ...
                hue, sat, val = cv2.split(cv2.cvtColor(rgb_img, cv2.COLOR_RGB2HSV))
                im_hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
                rgb_img = cv2.cvtColor(im_hsv, cv2.COLOR_HSV2RGB)

            # 重新合并图像
            labels["img"] = np.concatenate([rgb_img, depth_img], axis=-1)
        else:
            # 原有的单图像处理逻辑
            if self.hgain or self.sgain or self.vgain:
...
class Albumentations:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        im = labels["img"]
        # 如果是RGB-D图像，只对前3个通道应用Albumentations变换
        if im.shape[2] > 3:
            rgb_img = im[..., :3]
            depth_img = im[..., 3:]
            ...
            if self.contains_spatial:
                ...
                if len(cls):
                    ...
                    new = self.transform(image=rgb_img, bboxes=bboxes, class_labels=cls)
